...
export default class InviteManager {
    ...
    public inviteConfig: { id: number, name: number, reward: [{ type: number, num: number }] }[];

    /**得到邀请信息 */
    public getInviteAwardData(): DataType.localData.InviteData[] {
        this.getInviteInfo();
        let inviteConfig = this.inviteConfig;
    ...
    public getInviteInfo() {
        let inviteInfo = Laya.LocalStorage.getJSON(GameMgr.getInstance().invite);
        if (inviteInfo) {
    ...
    public getInviteAward(id: number, num: number) {
